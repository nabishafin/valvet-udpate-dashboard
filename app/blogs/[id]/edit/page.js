...
export default function EditBlogPage() {
  ...
  const { data: response, isLoading: isLoadingBlog } = useGetBlogByIdQuery(id, { skip: !id });
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-16">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between bg-[#fcfbf9]/90 backdrop-blur-md pb-4 pt-2 -mx-8 px-8 border-b border-zinc-100/50">
        <div className="flex items-center gap-4 min-w-0">
          <Link href="/blogs" className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-zinc-100 text-zinc-400 hover:text-zinc-900 transition-colors flex-shrink-0">
            <ArrowLeft size={20} />
          </Link>
          <div className="min-w-0">
            <p className="text-[13px] font-bold uppercase tracking-widest text-maroon mb-0.5">Blog</p>
            <h1 className="text-2xl font-black text-zinc-900 tracking-tight line-clamp-1">
              {formData.title ? `Edit: ${formData.title}` : 'Edit Post'}
            </h1>
          </div>
        </div>
        <button
          ...
          className="bg-maroon text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-maroon/90 transition-all shadow-lg shadow-maroon/20 active:scale-95 disabled:opacity-60 flex-shrink-0"
        >
          ...
          <div className="p-8 space-y-5">
            ...
            <div className="pt-2 border-t border-zinc-100">
              <button
                ...
                className="flex items-center justify-between w-full p-4 bg-zinc-50 rounded-xl border border-zinc-200 hover:border-zinc-300 transition-colors"
              >
